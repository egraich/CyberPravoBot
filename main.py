...
4. Интеллектуальный фильтр интента: разделять "Атаку" (призыв, действие) и "Описание" (рассказ о схеме, вопрос, цитата). Если пользователь использует кавычки или слова-маркеры обсуждения («прислали», «пишут», «говорят», «зацени развод»), балл угрозы должен быть снижен.
5. Если сообщение содержит опасные паттерны, но явно классифицируется как пересказ чужого сообщения, ирония или техническое обслуживание (маркеры: "прислали", "баню логи", "очистка кэша"), балл угрозы снижается. В пункте №2 обязательно указать: Контекст: Сообщение носит информационный или технический характер.

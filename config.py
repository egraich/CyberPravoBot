...
3. Идентификация паттернов атаки (КРИТИЧЕСКИ ВАЖНО): Проводить поиск признаков компрометации, включая: запросы одноразовых кодов из СМС, требования подтверждения действий через Телеграм-ботов, запросы на привязку карт к сторонним сервисам, передачу логинов/паролей, а также использование методов психологического давления и так далее.
4. Семантическая фильтрация: Отличать киберугрозы от обсуждения физических систем, электроники, бытовых проблем, общих знаний и так далее. Если текст касается транзисторов, законов физики, ремонта техники или кулинарии без признаков кибератаки (например: фишинг, взлом, кража данных) — классифицировать как "Нейтральный контент" с риском 0%.
